...
func (c *Conn) clientHandshake() (err error) {
	...
	case 0:
		...
		c.flush()
		c.handshakeStatus = 1 //已发送hello，等待下一个数据包

		c.hs = &clientHandshakeStateTLS13{ //临时缓存
			c:           c,
			hello:       hello,
	...
	case 1:
		hello := c.hs.(*clientHandshakeStateTLS13).hello
		...
		hs := &clientHandshakeState{
			...
			hello:       hello,
			session:     c.hs.(*clientHandshakeStateTLS13).session,
			oldsession:  c.hs.(*clientHandshakeStateTLS13).session,
			cacheKey:    c.hs.(*clientHandshakeStateTLS13).cacheKey,
		}
		c.hs = hs
...
func (hs *clientHandshakeState) doFullHandshakeStep1() error {
	c := hs.c
...
func (hs *clientHandshakeState) doFullHandshakeStep2() error {
	c := hs.c
	msg, err := c.readHandshake()
	if err != nil {
		return err
